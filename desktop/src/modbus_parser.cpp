/*
 * VoltGuard - Week 2: Modbus/TCP Protocol Parser (Bridge-ready)
 * ---------------------------------------------------------------
 * Same parsing logic as Week 1, plus a --json output mode.
 *
 * Week 1 only printed human-readable text. Week 2 needs the physics
 * engine (Python) to consume each parsed command, so this version can
 * also emit one JSON object per line (JSON Lines format) -- easy for
 * network_physics_bridge.py to read via subprocess/stdout piping.
 *
 * Build:
 *   g++ -std=c++17 -O2 -o modbus_parser modbus_parser.cpp
 *
 * Run (human-readable, same as Week 1):
 *   ./modbus_parser traffic_log.bin
 *
 * Run (JSON Lines, for the Week 2 bridge):
 *   ./modbus_parser traffic_log.bin --json
 */
...
// Minimal hand-rolled JSON emission -- no external JSON library needed
// for a handful of known numeric/string fields.
static std::string frame_to_json(int frame_number, const ModbusFrame& f) {
    ...
    oss << "{"
        ...
        << "\"register_name\":\"" << register_name(f.register_address) << "\","
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: " << argv[0] << " <modbus_log_file> [--json]\n";
        return 1;
    }
    ...
    while (offset < buf.size()) {
        ...
        if (json_mode) {
            // One JSON object per line (JSON Lines) -> stdout only.
            // This is what network_physics_bridge.py reads.
            std::cout << frame_to_json(frame_count, frame) << "\n";
        } else {
    ...
    if (!json_mode) {
        ...
        std::cerr << "Parsed " << frame_count << " frame(s) to JSON (stderr status message).\n";
    }
